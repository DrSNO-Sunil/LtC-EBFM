...
function [A] = func_snowmodel_With_Percolation_Blocking(C,A,clim,dt,grid,time,phys)
...
temp = (cumsum(A.subD.*A.subZ,2) - 0.5*A.subD.*A.subZ);
Psload(cond) = temp(cond);
Visc(cond) = 9d5.*exp(0.08*(C.T0-A.subT(cond))+0.021.*A.subD(cond));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% settings for barrier blocking percolation (impermeable layer)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
C.barrier_min_density = 705;     % SET MINIMUM DENSITY (KG M-3) FOR WATER BARRIER
C.barrier_min_thickness = 0.59;   % SET MINIMUM THICKNESS (M) FOR WATER BARRIER

barrier_thickness = zeros(grid.gpsum,1);
cond_blocked = false(grid.gpsum,1);
avail_W_loc_blocked = zeros(grid.gpsum,1);
A.subBLOCKED = false(grid.gpsum,grid.nl);

for n=1:nl
    barrier_thickness(A.subD(:,n)>C.barrier_min_density,1) = barrier_thickness(A.subD(:,n)>C.barrier_min_density,1) + A.subZ(A.subD(:,n)>C.barrier_min_density,n);
    barrier_thickness(A.subD(:,n)<=C.barrier_min_density,1) = 0;
    cond_blocked(barrier_thickness>C.barrier_min_thickness,1) = true;
    A.subBLOCKED(:,n) = cond_blocked;
    avail_W_loc(~cond_blocked,1) = avail_W_loc(~cond_blocked,1) + carrot(~cond_blocked,n);
    avail_W_loc(cond_blocked,1) = 0;
    avail_W_loc_blocked(cond_blocked,1) = avail_W_loc_blocked(cond_blocked,1) + carrot(cond_blocked,n);
...
avail_W = avail_W_loc + avail_W_loc_blocked;
