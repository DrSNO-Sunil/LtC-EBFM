...
for c = 1:maxIter
    ...
    LHF    = func_flux_LHF(C,tt1,clim,mask);
    SHF    = func_flux_SHF(C,tt1,clim,mask);
    GHF    = func_flux_GHF(tt1,A,GHF_C,mask);

	ebal_tt1 = SWin - SWout + LWin - LWout + SHF + LHF + GHF;
    
    LWout  = func_flux_LWout(C,ttmid);
    LHF    = func_flux_LHF(C,ttmid,clim,mask);
    SHF    = func_flux_SHF(C,ttmid,clim,mask);
    GHF    = func_flux_GHF(ttmid,A,GHF_C,mask);
